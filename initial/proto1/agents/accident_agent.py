...
class AccidentAgent:
    ...
    def handle_image(self, image_path, camera_id=None, notify_email=None):
        ...
        # 2. Lookup location (fallback default if camera_id is None or not found)
        loc = self.loc_agent.get_location(camera_id)
        lat, lon = loc["lat"], loc["lon"]
        address = loc.get("address", "Unknown Location")

        # 3. Allocate resources (ambulance, police, fire)
        allocated = self.allocator.allocate(incident_loc={"lat": lat, "lon": lon})

        # 4. Send notification email with annotated image attached
        ...
        summary = (
            ...
            f"Resources: {', '.join([r['type'] + ':' + r['id'] for r in allocated])}.\n"
